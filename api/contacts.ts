...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "POST") {
    ...
    try {
      await sql.execute(`CREATE TABLE IF NOT EXISTS contact_submissions (
      ...
      const [result] = await sql.execute<any>(
        ...
        [String(name), String(email), subject ? String(subject) : null, String(message)]
      );
      const [rows] = await sql.execute<any[]>("SELECT id, name, email, subject, message, submission_date as \"submissionDate\" FROM contact_submissions WHERE id = ?", [result.insertId]);
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      `);
      const [contacts] = await sql.execute<any[]>(`
